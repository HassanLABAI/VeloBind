...
from src.models.ensemble import run_oof, TargetScaler, _USE_GPU
...
os.environ["LIGHTGBM_GPU_WARNING"] = "0"
...
def quick_train(X_tr, y_tr, X_te, seed=42):
    """Fast single-LGBM 5-fold CV for ablation"""
...
def main():
    ...
    parser.add_argument("--skip-ablation", action="store_true",
                        help="Load cached ablation winner from models/best_cfg.json "
                             "and skip the 4-hour ablation. Use when features haven't changed.")
    args = parser.parse_args()

    print("=" * 65)
    print("VELOBIND - Step 3: Training + Ablation")
    print("=" * 65)
    ...
    cfg_cache = model_dir / "best_cfg.json"
...
# ══════════════════════════════════════════════════════════════════
    ...
    if args.skip_ablation and cfg_cache.exists():
        print(f"\n[Ablation — SKIPPED, loading cached winner from {cfg_cache}]")
        with open(cfg_cache) as f:
            cached = json.load(f)
        best_cfg_kwargs = cached["kwargs"]
        best_name       = cached["name"]
        best_R          = cached["R"]
        ablation_rows   = cached.get("rows", [])
        print(f"  Best config: '{best_name}'  (R={best_R:.4f})")
    else:
        ...
        with open(cfg_cache, "w") as f:
            json.dump({"name": best_name, "R": best_R,
                       "kwargs": best_cfg_kwargs, "rows": ablation_rows}, f, indent=2)
        print(f"  Ablation winner cached → {cfg_cache}")
...
# ══════════════════════════════════════════════════════════════════
    ...
    best_preds = max(
        ...
        key=lambda p: evaluate(p, y_test)['R']
    )
